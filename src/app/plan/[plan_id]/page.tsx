...
export default function PlanPage() {
  ...
  const planId = params?.plan_id ? String(params.plan_id) : "";
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!stored) {
        setError("Plan not found in history.");
        setLoading(false);
        return;
      }
      const parsed = JSON.parse(stored) as HistoryItem[];
      const item = parsed.find((entry) => entry.id === planId);
      if (!item) {
        setError("Plan not found in history.");
        setLoading(false);
        return;
    ...
    const timeout = setTimeout(() => {
      try {
        ...
        const next = parsed.map((item) =>
          item.id === planId ? { ...item, plan, prompt } : item
        );
  ...
  function updateStory(
    epicIndex: number,
    storyIndex: number,
    updates: Partial<Plan["epics"][number]["stories"][number]>
  ) {
    if (!plan) return;
  ...
  function addSubtask(epicIndex: number, storyIndex: number, taskIndex: number) {
    if (!plan) return;
    const newSubtaskIndex =
      plan.epics[epicIndex].stories[storyIndex].tasks[taskIndex].subtasks.length;
    const epicKey = `epic-${epicIndex}`;
    const storyKey = `story-${epicIndex}-${storyIndex}`;
  ...
  async function handleChatSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const nextMessages = [...chatMessages, { role: "user", content: instruction }];
    ...
    try {
      ...
      if (payload.message) {
        setChatMessages((prev) => [...prev, { role: "assistant", content: payload.message }]);
      }
      setEditMode(false);
    } catch (err) {
    ...
    return (
      <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-10 px-6 py-12">
        <header className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-ink-500">Plan</p>
            <h1 className="text-3xl font-semibold text-ink-900">Plan not found</h1>
            <p className="mt-2 max-w-2xl text-sm text-ink-600">
              {error || "We couldn’t locate this plan in your local history."}
            </p>
          </div>
          <TopNav />
        </header>
        <Link
          href="/"
          className="w-fit rounded-full border border-ink-200 bg-white px-4 py-2 text-xs font-semibold text-ink-700"
        >
          Back to planner
        </Link>
      </main>
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-5xl flex-col gap-10 px-6 py-12">
      ...
      <section className="rounded-3xl border border-ink-100 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-2">
          <p className="text-xs uppercase tracking-[0.2em] text-ink-500">Assistant</p>
          <h2 className="text-2xl font-semibold text-ink-900">Refine this plan</h2>
          <p className="text-sm text-ink-600">
            Describe the updates you want and the assistant will revise the plan while keeping the JSON schema.
          </p>
        </div>
        <form className="mt-4 flex flex-col gap-3" onSubmit={handleChatSubmit}>
          <textarea
            className="min-h-[140px] w-full rounded-2xl border border-ink-200 bg-ink-50/40 p-4 text-sm text-ink-900 focus:border-ink-400 focus:outline-none focus:ring-2 focus:ring-ink-100"
            placeholder="Ask for edits, add new tasks, adjust scope, or reassign work..."
            value={chatInput}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {chatLoading ? "Updating plan..." : "Update plan"}
            </button>
              ...
              <div
                ...
                className={`rounded-2xl border p-4 text-sm ${
                  message.role === "assistant"
                    ...
                    : "border-ink-100 bg-ink-50/60 text-ink-800"
                }`}
              >
            ...
            return (
              <div
                ...
                className={`rounded-3xl border border-ink-100 bg-white p-6 shadow-sm transition-colors transition-shadow duration-700 ${
                  highlightItems[epicKey] ? "bg-amber-50/80 ring-2 ring-amber-200" : ""
                }`}
              >
                <button
                  type="button"
                ...
                >
                  <div className="flex-1">
                    <p className="text-xs uppercase tracking-[0.2em] text-ink-500">Epic</p>
                    {editMode ? (
                    ...
                    {epic.stories.map((story, storyIndex) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <div className="flex-1">
                              <p className="text-xs uppercase tracking-[0.2em] text-ink-500">Story</p>
                              {editMode ? (
                                <div className="mt-2 flex flex-col gap-2">
                                  <input
                                    className="w-full rounded-xl border border-ink-200 px-3 py-2 text-sm text-ink-900"
                                ...
                                <div>
                                  <h4 className="text-lg font-semibold text-ink-900">{story.title}</h4>
                                  {story.description && (
                                    <p className="mt-1 text-sm text-ink-600">{story.description}</p>
                                  )}
                              ...
                              {story.tasks.map((task, taskIndex) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    >
                                      <div className="flex-1">
                                        <p className="text-xs uppercase tracking-[0.2em] text-ink-500">Task</p>
                                        {editMode ? (
                                          <div className="mt-2 flex flex-col gap-2">
                                            <input
                                              className="w-full rounded-xl border border-ink-200 px-3 py-2 text-sm text-ink-900"
                                          ...
                                          <div>
                                            <h5 className="text-sm font-semibold text-ink-900">{task.title}</h5>
                                            {task.description && (
                                    ...
                                    {!editMode && (
                                      <button
                                        ...
                                        onClick={() => setActiveTask({ epicIndex, storyIndex, taskIndex })}
                                      >
                                        View details
                                      </button>
                                    )}
                                          ...
                                          {editMode ? (
                                            <div className="mt-2 flex flex-col gap-2">
                                              {requirements.map((req, reqIndex) => (
                                                <div key={`req-${reqIndex}`} className="flex items-center gap-2">
                                                  <input
                                              ...
                                              {requirements.length === 0 && (
                                                <p className="text-xs text-ink-500">No requirements yet.</p>
                                              )}
                                            </div>
                                          ) : (
                                          ...
                                          {editMode ? (
                                            <div className="mt-2 flex flex-col gap-2">
                                              ...
                                              {task.subtasks.length === 0 && (
                                                <p className="text-xs text-ink-500">No subtasks yet.</p>
                                              )}
                                            </div>
                                          ) : (
                                            <ul className="mt-2 list-disc pl-5 text-sm text-ink-600">
                                              {task.subtasks.length > 0 ? (
                                                task.subtasks.map((subtask, subtaskIndex) => (
                                                  <li key={`sub-${subtaskIndex}`}>
                                                    <span className="font-medium text-ink-700">{subtask.title}</span>
